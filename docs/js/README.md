...
We include a generator for components, pages, sagas, routes and selectors.
...
## Architecture: `components` and `pages`

We adopted a split between stateless, reusable components called (wait for it...)
`components` and stateful parent components called `pages`.
